...
public class SellerService {
    ...
    public String createSeller(CreateSellerRequest request) {
        ...
        if (sellerRepository.findByUserId(request.getSellerId()).isPresent())
            throw new IllegalArgumentException("이미 등록된 아이디 입니다..");
        if (sellerRepository.findByUserPhoneNumber(request.getSellerPhoneNumber()).isPresent())
    ...
    @Transactional
    public SellerResponse readSeller(FindSellerIdRequest request) {
        return sellerRepository.findByUserNameAndUserPhoneNumber(request.getSellerName(), request.getSellerPhoneNumber())
                .orElseThrow(IllegalArgumentException::new);
    }

    @Transactional
    public String updateSellerName(UpdateSellerNameRequest request) {
        Seller seller = sellerRepository.findByUserId(request.getSellerId())
                .orElseThrow(IllegalChannelGroupException::new);
        if (!request.getSuperAdminPassword().equals("tempPw"))
            throw new IllegalArgumentException("비밀번호를 확인해주세요.");
        String currentUserName = seller.getSellerName();
        seller.updateSellerName(request.getNewSellerName());
        return currentUserName + " 유저 이름이 " + seller.getSellerName() + "으로 변경되었습니다.";
    }
    ...
    public String updateSellerPhoneNumber(UpdateSellerPhoneNumberRequest request) {
        Seller seller = sellerRepository.findByUserId(request.getSellerId())
                .orElseThrow(IllegalArgumentException::new);
        if (sellerRepository.findByUserPhoneNumber(request.getNewSellerPhoneNumber()).isPresent())
            throw new IllegalArgumentException("이미 등록된 핸드폰번호 입니다.");
    ...
    public String deleteSeller(DeleteSellerRequest request) {
        ...
        if (!request.getSuperAdminPassword().equals("tempPw"))
            throw new IllegalArgumentException("비밀번호를 확인해주세요.");
        String deleteUser = seller.getSellerName();
